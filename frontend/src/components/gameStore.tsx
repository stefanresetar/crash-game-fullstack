...
export const CrashCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (status === 'running' || status === 'crashed') {
        ...
        ctx.beginPath();
        ctx.moveTo(40, h - 40);
        ctx.quadraticCurveTo(40, endY, endX, endY);
        ctx.lineWidth = 4;
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden bg-radial-dark rounded-lg border border-gray-700 shadow-2xl">
...
const StatusOverlay = () => {
    const { status, currentMultiplier, timeLeft } = useGameStore();

    if (status === 'waiting') {
        return (
            <div>
                <div className="text-2xl font-bold text-gray-500 tracking-[4px] uppercase mb-2">SLEDEĆA RUNDA</div>
                <div className="text-[50px] font-bold text-gray-400">{timeLeft.toFixed(1)}s</div>
            </div>
        );
    }
    ...
    if (status === 'running') {
        ...
        return (
            <div>
                <div className="text-2xl font-bold text-gray-500 tracking-[4px] uppercase mb-2">U TOKU</div>
    ...
    return <div className="text-white">UČITAVANJE...</div>;
};